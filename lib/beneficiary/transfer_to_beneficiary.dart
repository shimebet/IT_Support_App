...
class _TransferToBeneficiaryPageState extends State<TransferToBeneficiaryPage> {
  ...
  void onBeneficiaryAccountSelected(String? accountNumber) {
    setState(() {
      ...
      beneficiaryName = beneficiaries
          .firstWhere((element) => element['accountNumber'] == accountNumber)['beneficiaryName'];
    });
  }
...
DropdownButtonFormField<String>(
  ...
  items: beneficiaries.map((beneficiary) {
    // Get the first word of the beneficiary name, handling potential null value
    String firstName = (beneficiary['beneficiaryName'] ?? '').split(' ').first;

    return DropdownMenuItem(
      value: beneficiary['accountNumber'], // Keep the value as account number
      child: Text('$firstName - ${beneficiary['accountNumber']}'), // Display first name and account number
    );
  }).toList(),
),
